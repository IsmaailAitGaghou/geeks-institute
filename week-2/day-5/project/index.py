...
def get_pagination_data(query, params, page, per_page=6):
    ...
    items = [dict(zip([desc[0] for desc in g.cur.description], row)) for row in g.cur.fetchall()]
...
def list_albums():
    ...
    return render_template('index.html', albums=pagination['items'], pagination=pagination, search_query=search_query)
...
def create_album():
    if request.method == 'POST':
        ...
        for artist_id in artist_ids:
            g.cur.execute("INSERT INTO album_artist (album_id, artist_id) VALUES (%s, %s)", (new_album_id, artist_id))
...
def details_album(album_id):
    ...
    """, (album_id,))
    artists = [dict(zip([desc[0] for desc in g.cur.description], row)) for row in g.cur.fetchall()]
...
def edit_album(album_id):
    if request.method == 'POST':
        ...
        g.cur.execute("UPDATE album SET title = %s, release_year = %s, genre_id = %s WHERE id = %s",
                      (title, release_year, genre_id, album_id))
        
        g.cur.execute("DELETE FROM album_artist WHERE album_id = %s", (album_id,))
        if artist_ids:
            for artist_id in artist_ids:
                g.cur.execute("INSERT INTO album_artist (album_id, artist_id) VALUES (%s, %s)", (album_id, artist_id))
    ...
    # GET request
    g.cur.execute("SELECT id, title, release_year, genre_id FROM album WHERE id = %s", (album_id,))
    ...
    return render_template('edit.html', type='album', item=album, genres=genres, artists=artists, selected_artist_ids=selected_artist_ids)
